...
const CalendarCells = ({ currentMonth, onDateClick, selectedDate }: { currentMonth: Date, onDateClick: (day: Date) => void, selectedDate: string }) => {
    ...
    while (day <= endDate) {
        for (let i = 0; i < 7; i++) {
            ...
            const cloneDay = day
            days.push(
                <div className={`w-full h-44 relative gap-1 border rounded pb-[12%] text-end ${format(currentMonth, 'M') !== format(day, 'M') ? 'bg-gray-100 text-gray-300' : 'cursor-pointer'}`} key={formattedDate} onClick={() => {
                    clickModal()
